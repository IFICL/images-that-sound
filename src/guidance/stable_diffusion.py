...
class StableDiffusionGuidance(nn.Module):
    ...
    def train_step(self, text_embeddings, pred_rgb, guidance_scale=100, as_latent=False, t=None, grad_scale=1, save_guidance_path:Path=None):
        # import pdb; pdb.set_trace()
        pred_rgb = pred_rgb.to(self.vae.dtype)
        if as_latent:
    ...
    def encode_imgs(self, imgs, generator=None):
        # imgs: [B, 3, H, W]
        imgs = 2 * imgs - 1
    ...
    def prompt_to_img(self, prompts, negative_prompts='', height=512, width=512, num_inference_steps=50, guidance_scale=7.5, latents=None, device=None, generator=None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--repo_id', type=str, default='runwayml/stable-diffusion-v1-5', help="stable diffusion version")
    ...
    sd = StableDiffusionGuidance(repo_id=opt.repo_id, fp16=opt.fp16)
    sd = sd.to(device)
